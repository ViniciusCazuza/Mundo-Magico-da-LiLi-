...
import { Cat, AlertCircle, RefreshCw, Book, MessageSquare, Calendar, Palette, User, ShieldCheck, Lock, LogOut, Sparkles, Moon, Sun, Cloud } from "lucide-react";

// Core Imports
import { Conversation, UsageLog, AppSection, AppTheme, CalendarEvent, ParentReport, Message } from "./core/types";
...
const AppShell = ({ profile, onLogout, sessionConv, onUpdateConversation }: AppShellProps) => {
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--color-success', tokens.colors.success || '#22C55E');
    root.style.setProperty('--color-warning', tokens.colors.warning || '#F59E0B');
    root.style.setProperty('--color-danger', tokens.colors.danger || '#EF4444');
    root.style.setProperty('--color-info', tokens.colors.info || '#3B82F6');
...
const ProfileGate = () => {
  ...
  const handlePinSubmit = () => {
    if (targetProfileId && IdentityManager.login(targetProfileId, pinInput)) setTargetProfileId(null);
    else { setPinInput(""); }
  };
  ...
  if (isBooting) {
    return (
      <div className="fixed inset-0 bg-[#F8F9FF] flex flex-col items-center justify-center z-[10000] overflow-hidden">
        <div className="absolute inset-0 opacity-20 animate-aurora pointer-events-none" style={{ background: 'linear-gradient(45deg, #818CF8, #F472B6, #FB923C)', backgroundSize: '400% 400%' }}></div>
  ...
  return (
    <div className="fixed inset-0 bg-white flex items-center justify-center p-6 z-[5000] overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-tr from-[#EEF2FF] to-[#FFF5F7] animate-aurora pointer-events-none" style={{ backgroundSize: '400% 400%' }}></div>
      ...
      {targetProfileId && (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center p-6 bg-slate-950/40 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-[3.5rem] p-12 shadow-2xl space-y-10 text-center border-4 border-white">
            ...
            <div className="flex justify-center gap-3">
               {[0,1,2,3].map(i => (
                 <div key={i} className={`w-4 h-4 rounded-full transition-all ${pinInput.length > i ? 'bg-indigo-500 scale-125' : 'bg-slate-200'}`}></div>
               ))}
            </div>
            <input 
              type="password" 
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => setTargetProfileId(null)} className="py-4 text-[10px] font-black uppercase text-slate-300 hover:text-indigo-500 transition-colors">Cancelar</button>
            </div>
          </div>
        </div>
      )}
...
root.render(<ErrorBoundary><ProfileGate /></ErrorBoundary>);
