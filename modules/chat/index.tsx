  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 relative bg-transparent overflow-hidden">
      <div className="absolute top-10 right-10 opacity-20 animate-mimi-float pointer-events-none">
        <Sparkles size={80} className="text-[var(--primary)]" />
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 md:px-12 py-10 space-y-10 no-scrollbar pb-40">
        {conversation.messages.map((m, i) => (
          <div key={`${m.timestamp}-${i}`} className={`flex items-end gap-4 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-fade-in`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-lg overflow-hidden transition-transform hover:scale-110 ${m.role === 'user' ? 'bg-[var(--primary)] text-[var(--text-on-primary)]' : 'bg-[var(--surface)] text-[var(--primary)] border border-[var(--border-color)] animate-float-slow'}`}>
              {m.role === 'user' ? (
            ...
            <div 
              className={`
                ...
                ${m.role === 'user' 
                  ? 'bg-[var(--primary)] text-[var(--text-on-primary)] rounded-[2.5rem] rounded-br-[0.5rem] shadow-[var(--shadow-base)]' 
                  : 'bg-[var(--surface)] backdrop-blur-md text-[var(--text-primary)] border border-[var(--border-color)] rounded-[2.5rem] rounded-bl-[0.5rem] shadow-[var(--shadow-base)]'
                }
              `}
        ...
        {internalProcessing && (
          <div className="flex justify-start items-center gap-4 animate-fade-in">
             <div className="w-12 h-12 rounded-2xl bg-[var(--surface)] border border-[var(--border-color)] flex items-center justify-center text-[var(--primary)] shadow-md animate-mimi-float">
                <Cat size={22} />
             </div>
             <div className="flex flex-col gap-2">
                <div className="flex gap-2 p-5 bg-[var(--surface)] backdrop-blur-md rounded-[2.5rem] border border-[var(--border-color)] shadow-sm">
      ...
      {!isReadOnly && (
        <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[var(--bg-app)] via-[var(--bg-app)]/80 to-transparent shrink-0">
          <div className="max-w-4xl mx-auto flex items-center gap-4 bg-[var(--surface)] backdrop-blur-2xl p-3 rounded-[3.5rem] shadow-[var(--shadow-elevated)] border-4 border-[var(--border-color)] focus-within:border-[var(--primary)] transition-all transform hover:-translate-y-1">
            <button className="p-4 text-[var(--text-muted)] hover:text-[var(--primary)] transition-colors hidden md:block">
              <Smile size={28} />
            </button>
            <input 
              value={inputValue}
              onChange={e => setInputValue(e.target.value)}
              onKeyDown={e => { if(e.key === 'Enter') handleMimiFlow(inputValue); }} 
            ...
            <div className="flex gap-2">
              ...
              <button 
                className="w-16 h-16 bg-[var(--primary)] rounded-full flex items-center justify-center text-[var(--text-on-primary)] shadow-lg hover:brightness-110 active:scale-90 transition-all group"
