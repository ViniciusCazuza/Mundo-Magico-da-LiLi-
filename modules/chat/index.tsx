    ...
    <div className="flex-1 flex flex-col min-h-0 relative bg-transparent overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 md:px-12 py-10 space-y-10 no-scrollbar pb-40">
        {conversation.messages.map((m, i) => (
          <div key={`${m.timestamp}-${i}`} className={`flex items-end gap-4 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-fade-in`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-lg overflow-hidden transition-transform hover:scale-110 ${m.role === 'user' ? 'bg-indigo-500 text-white' : 'bg-white text-indigo-500 border border-indigo-100 animate-float-slow'}`}>
              {m.role === 'user' ? (
            ...
            <div 
              className={`
                ...
                ${m.role === 'user' 
                  ? 'bg-indigo-500 text-white rounded-[2.5rem] rounded-br-[0.5rem] shadow-indigo-500/20' 
                  : 'bg-white/90 backdrop-blur-md text-slate-800 border border-white rounded-[2.5rem] rounded-bl-[0.5rem] shadow-slate-200/50'
                }
              `}
        ...
        {internalProcessing && (
          <div className="flex justify-start items-center gap-4 animate-fade-in">
             ...
             <div className="flex flex-col gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-500/60 ml-3 animate-pulse">
                  Mimi está pensando...
                </span>
             </div>
          </div>
      ...
      {!isReadOnly && (
        <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[var(--bg-app)] via-[var(--bg-app)]/80 to-transparent shrink-0">
          <div className="max-w-4xl mx-auto flex items-center gap-4 bg-white/90 backdrop-blur-2xl p-3 rounded-[3.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border-4 border-white focus-within:border-indigo-400 focus-within:shadow-[0_0_30px_rgba(129,140,248,0.3)] transition-all">
            <button className="p-4 text-indigo-400 hover:text-indigo-600 transition-colors hidden md:block">
              <Smile size={28} />
            </button>
            <input 
              value={inputValue}
              ...
              className="flex-1 bg-transparent px-4 py-4 outline-none font-hand text-3xl placeholder:opacity-30 text-slate-800" 
              disabled={internalProcessing}
            />
            <div className="flex gap-2">
              <button 
                className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-500 hover:bg-indigo-200 transition-colors active:scale-95"
                disabled={internalProcessing}
              >
                <Mic size={28} />
              </button>
              <button 
                className="w-16 h-16 bg-indigo-500 rounded-full flex items-center justify-center text-white shadow-lg hover:scale-110 active:scale-90 transition-all group"
